...
def execute_user_code(task_id: int | None, user_code: str):
    """
    Выполняет код пользователя.
    Если task_id есть - запускает тесты, если нет - просто выполняет код в песочнице.
    """
    ...
    try:
        # РЕЖИМ ПЕСОЧНИЦЫ: task_id отсутствует, равен None или пустая строка
        if task_id is None or task_id == '':
            # 1. Записываем код пользователя
            with open(user_file, 'w', encoding='utf-8') as f:
                f.write(user_code)

            # 2. Просто запускаем код пользователя напрямую
            start_time = time.time()

            # 3. Выполняем код пользователя
            result = subprocess.run(
                ['python', user_file],
                capture_output=True,
                text=True,
                timeout=10,  # Лимит времени для песочницы
                cwd=temp_dir
            )
            ...
            if result.returncode == 0:
                return {
                    ...
                    'output': clean_user_output(result.stdout),
                    'error': clean_user_output(result.stderr),
                    'sandbox': True,  # Флаг, что это режим песочницы
                    'message': 'Код успешно выполнен в песочнице'
                }
            else:
                # Пытаемся извлечь полезную информацию об ошибке
                error_msg = result.stderr
                if "SyntaxError" in error_msg:
                    # Извлекаем информацию о синтаксической ошибке
                    lines = error_msg.split('\n')
                    simplified_error = "\n".join([line for line in lines if 'File' not in line and '^' not in line])
                    error_msg = f"Синтаксическая ошибка: {simplified_error[:500]}"
                elif "NameError" in error_msg:
                    error_msg = "Ошибка имени: возможно, переменная или функция не определена"
                elif "IndentationError" in error_msg:
                    error_msg = "Ошибка отступов: проверьте табуляцию и пробелы"
                ...
                return {
                    ...
                    'execution_time_ms': execution_time,
                    'output': clean_user_output(result.stdout),
                    'error': clean_user_output(error_msg)[:1000],
                    'sandbox': True,
                    'message': 'Ошибка при выполнении кода'
                }

        # РЕЖИМ ТЕСТИРОВАНИЯ: task_id есть (проверяем, что это число)
        else:
            # Преобразуем task_id в число, если это возможно
            try:
                task_id_int = int(task_id)
            except (ValueError, TypeError):
                # Если task_id не число, считаем это песочницей
                return {
                    'success': False,
                    'error': f"Некорректный task_id: {task_id}",
                    'status': 'error',
                    'sandbox': True
                }

            from code_cup.models import Task
            task = Task.objects.get(id=task_id_int)
            ...
            return {
                ...
                'sandbox': False,
                'output': clean_user_output(result.stdout)  # Вывод пользователя (если есть)
            }

    except subprocess.TimeoutExpired:
        return {
            'success': False,
            'error': f'Лимит времени ({10 if task_id in [None, ""] else task.time_limit} сек.) превышен',
            'status': 'timeout',
            'sandbox': task_id in [None, ""]
        }
